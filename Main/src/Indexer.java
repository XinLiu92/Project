...
public class Indexer {
    ...
    public void indexQue(Que que,Ans ans) throws IOException {
        if(que != null){
            ...
            doc.add(new TextField("answer",ans.getAnsBody(),Field.Store.YES));
            doc.add(new StringField("maxtf", Integer.toString(maxTF), Field.Store.YES));
            writer.addDocument(doc);
        }
    }

    public void rebuildIndexes(List<Que> list, List<Ans> ansList) throws IOException {
        getIndexWriter();
        if (!list.isEmpty()){
//            for (Que que : list){
//                indexQue(que);
//            }
            ...
            for (int i = 0; i < list.size();i++){
                indexQue(list.get(i),ansList.get(i));
            }
            closeIndexWriter();
        }
    }
